...
- Packaged build (`Install Add-on From File`), TB 154, 2026-09-01: full `docs/smoke-test-0.3.0.md` passed on b1127e0 (the first packaged run, on 13a85e7, found the reading-side toggle dead — a top-level `const` in `text.js` broke re-injection; fixed with a run-twice test), Cancel items 1b/11b on e1d173f, tooltip and selection tip on 67782f0. `node --test`: 35/35.
