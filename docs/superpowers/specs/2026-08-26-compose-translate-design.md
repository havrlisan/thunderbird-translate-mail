...
- **`composeInsert`** `{ texts }` → `{ inserted }`. `texts` must have exactly as many items as collected, and each Range's current HTML must equal the snapshot taken at collect time (else `inserted: false`). Per Range: each returned item is parsed with `DOMParser` and sanitized — `script`/`style`/`iframe`/`object`/`embed`/`link`/`meta`/`base` removed, `on*` attributes and `javascript:` URLs (incl. `xlink:href`) stripped — placeholders are replaced by their lifted subtrees (a placeholder the Provider dropped is appended at the end of the run), the items are joined, the Range is selected and `document.execCommand('insertHTML', false, html)` writes it. `inserted` is false if `execCommand` reports failure; nothing is written directly.
