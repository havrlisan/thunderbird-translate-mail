...
- A newly displayed message resets the tab's button state (label/tooltip).
...
- Provider/network error → button back to "Translate", popup window explains the failure (status-specific text + raw response), message untouched.
...
- Failures open a popup window (`src/dialog.html`, since 0.5.0 also the large-run confirmation) with a status-specific explanation. Under the System-auto theme it stays light even when Thunderbird is dark: `theme.getCurrent()` returns no colours and extension pages get a light `prefers-color-scheme`. Accepted for v1.
