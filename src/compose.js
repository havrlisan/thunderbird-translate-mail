...
let busy = false;
let confirmed = false; // the popup is asking "Translate N characters?": Translate means yes, Cancel drops it
let selection = false;

function render(r) {
  busy = !!r.busy;
  confirmed = !!r.confirm;
  if ('selection' in r) selection = !!r.selection;
  $('go').disabled = false;
  $('go').textContent = t(busy ? 'cancel' : selection ? 'translateSelection' : 'translate');
  $('no').hidden = !confirmed;
  $('no').textContent = t('cancel');
  $('hint').hidden = selection;
